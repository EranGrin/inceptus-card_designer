odoo.define('card_design.editor', function (require) {
"use strict";
...
options.registry["width-x"] = options.Class.extend({
    start: function () {
        ...
        this.$overlay.find(".oe_handle").on('mousedown', function (event) {
            ...
            $body.on("mousemove.card_design_width_x", function (event) {
                event.preventDefault();
...
ajax.loadXML("/card_design/static/src/xml/card_design.xml", core.qweb);
    ...
    compute_snippet_templates: function (html) {
        ...
        var themes_params = _.map($themes, function (theme) {
            ...
            _.each(images_info, function (info) {
                info = _.defaults(info, images_info.all, {module: "card_design", format: "jpg"});
            });
            return {
                name: name,
                className: classname || "",
         ...
         */
        var $dropdown = $(core.qweb.render("card_design.theme_selector", {
         ...
         */
        // var $mail_themes_upgrade = $dropdown.find(".o_card_design_themes_upgrade");
